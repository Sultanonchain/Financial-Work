...
1. **Subsidy-adjusted DCF** — `DCF + (subsidy_PV ÷ shares)`.  For tickers with named, citable government capital (CHIPS Act grants, DPA Title III contracts), we add the announced dollar amount to enterprise value before per-share conversion.  INTC = $19.5B ($8.5B grant + $11B subsidized loan PV); MU = $6.1B; TXN = $1.6B; etc.
2. **Book-value floor** — `tangible_book_per_share × tier_multiplier` (1.5× semi/critical, 2.0× defense, 1.4× energy).  Reflects the principle that the U.S. won't let strategically-anchored assets trade at distressed book value.  CHIPS Act foundries the DoD designates as "trusted foundries" (INTC, MU, GFS) get the **defense-grade 2.0× multiplier** — they're sole-source for strategic platforms.
...
Verdict phrasing tiers when survival_floor is set:

- **MOS within −10%** → *"Strategic re-rating opportunity"* (floor lifts the math meaningfully into fair-value territory).
- **MOS between −10% and −25%** → *"Premium for strategic floor"* (DCF undercounts the government backstop; modest market premium).
- **MOS worse than −25%** → *"Market is paying X% above the strategic floor"* — honest framing for cases like Intel at an ATH, where even acknowledging CHIPS Act / DoD backing, the market is paying for outcomes most companies don't deliver. Math is math; strategic backing matters but doesn't justify any price.
...
## Lynch Lens — Per-Ticker Haiku Verdict

When `ANTHROPIC_API_KEY` is set, every ticker also gets a one-shot, Peter-Lynch-flavored verdict generated by `claude-haiku-4-5-20251001`.  The model receives the assembled snapshot — price, intrinsic value, margin of safety, the quality scorecard, risk labels, and the strategic-asset profile — and returns:

- **Lynch category**: Slow Grower, Stalwart, Fast Grower, Cyclical, Turnaround, or Asset Play.
- **Verdict**: Buy / Accumulate / Hold / Watch / Avoid.
- **Thesis**: a single Lynch-flavored sentence (≤ 220 chars).
- **Bull / bear case**: 2–3 punchy points each.
- **Sovereign backstop note**: when a strategic asset (CHIPS Act recipient, defense prime, energy sovereignty, etc.) applies, Haiku is explicitly told to soften the verdict and explain the floor — so a name like INTC or MU does not get a "weak FCF → Avoid" reading when the real thesis is government-backed re-shoring.

The verdict is cached 24 hours per `(ticker, $price-bucket, MOS-bucket)` and gracefully disabled when no API key is configured.

---

## Risk Analysis Card

The Risk card surfaces signals already produced by the analyzer, in one place: IV confidence (high/moderate/low), DCF confidence warnings (FCF inconsistency, leverage, cyclicality, thin margin, revenue decline), the WACC risk surcharge flag, news-driven risk labels, and policy headwinds vs tailwinds.  When nothing fires it shows a single "Clean read — no material risk flags" line instead of a sea of empty pills.

---

## Congressional Trades (STOCK Act PTRs)

The Congressional Trades card pulls Periodic Transaction Reports filed under the STOCK Act by members of the House and Senate, via the free community JSON mirrors at `house-stock-watcher-data` and `senate-stock-watcher-data` (S3).  We filter to the 180-day window for the requested ticker and surface up to six most-recent rows: date, member, chamber, buy/sell, and disclosed amount range.  Cached server-side for 6 hours so the community feeds aren't hammered.

This complements the existing Form 4 (insider) and 13F (institutional) filings and rounds out the "who is actually buying this name?" question with the third public-disclosure channel.
